...
#     # print(f"Past 26 active dates: {df['date'].tolist()}")   ############
...
#         symbols = [result[0] for result in tckr_symb_list]    # modified gpt
#         # return [result[0] for result in tckr_symb_list]   
#     # print(f"FO equity symbols list: {symbols}")   ###########
#     return symbols   ##########
...
#     print("Fetching Nifty 500 symbols list")
#     nifty_500 = 'https://archives.nseindia.com/content/indices/ind_nifty500list.csv'
...
#             output.add(symbol)
#     filename.unlink()
#     # print(f"Nifty 500 symbols: {output}")
#     return output


# def get_eq_security_bhav_data(current_date):
#     print(f"Fetching EQ security bhav data for date: {current_date}")
...
#     print(f"Fetching security bhav data for date: {current_date}")
#     with next(get_db()) as db_session:
#         data = db_session.query(
...
#     # print(f"Fetching Nifty index data for date: {data_date}")
#     with next(get_db()) as db_session:
#         index_data = db_session.query(
#             CMIndexData).filter(CMIndexData.index_name == 'Nifty 50', CMIndexData.index_date == data_date).first()
#         # print(f"Fetched Nifty index data: {index_data}")
#         return index_data
        


# def get_ad_and_dd(past_26_active_dates: List[datetime]) -> dict:
#     print(f"Getting Accumulation and Distribution (AD) and Distribution (DD) data for the past 26 active dates")
#     output = {}
...
#                 print(f"Error processing symbol {t_symbol}: {e}")
#                 # print(e)
#                 # print(e.__str__())
#                 continue
...
#         i += 1
#     print(f"Accumulation and Distribution data: {output}")
#     return output


# def get_rs(past_26_active_dates: list, days: int):
#     print(f"Calculating RS for {days} days")
...
#     print("old index data : ", old_index_data)
#     today_index_data = get_nifty_index_data(today_datetime_fmt)
#     print("today index data : ", today_index_data)
...
# #             # Skip non-EQ series unless symbol is in Nifty 500
# #             if series.strip() != "EQ" and (symbol not in nifty_500_data):
# #                 continue

# #             try:
# #                 # Calculate qnty_per_trade with a fallback for invalid data
# #                 if no_of_trades and no_of_trades > 0:
# #                     qnty_per_trade = int(ttl_trd_qnty) / int(no_of_trades)
# #                 else:
# #                     qnty_per_trade = 0  # Default value if no_of_trades is invalid
# #             except Exception as e:
# #                 qnty_per_trade = 0  # Handle unexpected errors

# #             try:
# #                 # Calculate turnover_per_trade with a fallback for invalid data
# #                 if no_of_trades and no_of_trades > 0:
# #                     turnover_per_trade = turnover / no_of_trades
# #                 else:
# #                     turnover_per_trade = 0  # Default value if no_of_trades is invalid
# #             except Exception as e:
# #                 turnover_per_trade = 0  # Handle unexpected errors

# #             # Add your logic to store or process the calculated values
# #             if symbol not in total_data:
# #                 total_data[symbol] = []

# #             total_data[symbol].append({
# #                 "date": data_date,
# #                 "qnty_per_trade": qnty_per_trade,
# #                 "turnover_per_trade": turnover_per_trade,
# #                 "deliv_qty": deliv_qty,
...
# # def get_the_past_26_active_dates(current_date: datetime):
# #     print(f"Getting the past 26 active dates for current date: {current_date}")
# #     # with next(get_db()) as db_session:
# #     #     return [i.date for i in db_session.query(
# #     #         SecurityiesBhavData.date).filter(
# #     #             SecurityiesBhavData.series.like('%EQ%'),
...
# # def get_fo_equity_symbols_list(current_date: datetime):
# #     print(f"Getting FO equity symbols list for date: {current_date}")
# #     with next(get_db()) as db_session:
# #         tckr_symb_list = db_session.query(FOUDIFFBhavData.tckr_symb).filter(
# #             FOUDIFFBhavData.trade_date == current_date).distinct().all()
# #         symbols = [result[0] for result in tckr_symb_list]
# #         print(f"FO equity symbols list: {symbols}")
# #         return symbols
...
# #             yesterday_data = {datum.symbol: datum for datum in yesterday_data}
# #             data = [data[-1], yesterday_data]
# #         print(f"Processing AD/DD for today: {today_date}, yesterday: {yesterday_date}")
# #         for t_symbol, t_datum in data[0].items():
# #             try:
# #                 t_closed_price = t_datum.close_price
# #                 t_volume = int(t_datum.total_trade_quantity)  # ttl_trd_qnty
...
# #     today_data_db = get_eq_security_bhav_data(today_date)
# #     today_data = {datum.symbol: datum for datum in today_data_db}

# #     for symbol, today in today_data.items():
# #         if symbol in old_data:
# #             old = old_data[symbol]
# #             try:
# #                 rs[symbol] = (today.close_price - old.close_price) / old.close_price
# #             except Exception as e:
# #                 print(f"Error calculating RS for {symbol}: {e}")
# #                 continue

# #     print(f"RS for {days} days: {rs}")
# #     return rs
...
def main(current_date: datetime, upload_to_remote_server=False):
    ...
    for datum in current_data:
        ...
        req_data['no_of_trades'] = datum.number_of_trades
        req_data['deliv_qty'] = datum.delivery_quantity
        req_data['qnty_per_trade'] = datum.total_trade_quantity / datum.number_of_trades
        try:
            req_data["trade_qty_deviation"] = int((
                (req_data['qnty_per_trade'] - total_data[datum.symbol]['avg_qnty_per_trade'])/total_data[datum.symbol]['avg_qnty_per_trade'])*100)
        except:
